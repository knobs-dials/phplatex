...
- If you get "convert: not authorized" when using it, this is likely due to an 2018 ImageMagick update that disabled PDF/PS conversions by default, apparently for security, and you need to [tweak its policy.xml to re-enable it](https://www.google.com/search?q=convert%3A+not+authorized+policy.xml).
...
If you care only about formulae on webpages, and don't care about the precise behaviour of compiled TeX, then you can get 95% of the way there, get _better_ indeability and copyability for simple things, and/or avoid a heavy depdendency and its security issues, by considering options such as:
