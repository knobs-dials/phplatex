...
- If in use you get an error like "convert: not authorized" or "attempt to perform an operation not allowed by the security policy", this is likely due to ImageMagick updates (2018 and later) that disabled PDF and/or PS conversions by default, apparently for security, and you need to [tweak its `policy.xml` to re-enable it](https://www.google.com/search?q=convert%3A+not+authorized+policy.xml).
